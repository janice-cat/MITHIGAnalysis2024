...
public:
  ...
  DataAnalyzer(const char *filename, const char *outFilename, const char *mytitle = "")
    ...
    outf->cd();
    nt = new TNtuple("nt", "D0 mass tree", "Dmass:Dgen:Dpt:Dy:Dalpha:Dchi2cl:DsvpvDistance:DsvpvDisErr");
    nt->SetAutoFlush(200000);   // flush baskets every ~200k entries (tune)
    nt->SetAutoSave(200000000); // autosave occasionally (bytes; tune)
